...
from inonit_spider import parser


def getUniqueId(response):
	return response.url.split('#')[0].split('/')[-1].split('pid-')[1].split('.')[0]

def parseLink(givenLink = "", forceRefresh = False):
	return parseLinkForSite( "inonit", parser, getUniqueId, givenLink, forceRefresh )
